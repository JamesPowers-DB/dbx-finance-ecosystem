...
-- Reads files from: /Volumes/${catalog}/${schema}/${raw_volume}/oracle_fusion/
--
...
CREATE OR REFRESH STREAMING TABLE ${schema}.bronze_po_headers_all
...
FROM STREAM read_files(
  "/Volumes/${catalog}/${schema}/${raw_volume}/oracle_fusion/po_headers_all_*.csv",
  format => "csv",
  header => true,
  inferColumnTypes => true
);

CREATE OR REFRESH STREAMING TABLE ${schema}.bronze_po_lines_all
...
AS SELECT
  *,
  ...
  _metadata.file_modification_time AS _ingested_at
FROM STREAM read_files(
  "/Volumes/${catalog}/${schema}/${raw_volume}/oracle_fusion/po_lines_all_*.parquet",
  format => "parquet"
);
