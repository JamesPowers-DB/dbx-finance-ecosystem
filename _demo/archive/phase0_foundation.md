...
- [x] Quick sanity checks in SQL editor (see `sql/pipeline_exploration/spend_overview.sql`):
  ```sql
  ...
  ```
- [x] Implement `ml/notebooks/99_validate_gold_vs_anchors.py` (currently a no-op stub wired as the final task in `build_lakehouse.yml`) — assert gold totals tie back to `_meta.dim_period_anchors` ±2%.

## Smoke tests
- [x] Anonymization audit: grep all UC table **contents** (not just code) for source filer name / original segment names.
...
- [x] Add a silver layer for HR if/when a second HR source appears.
- [x] Salary raises — generator doesn't model raises; ~3%/quarter raise events would add SCD2 churn + wage-inflation trends.
- [x] Role-family per segment — currently each segment pulls from any of 9 job families; could bias for realism.
- [x] Tie-out check for HR — extend `99_reconcile.py` to assert quarter-end active counts within ±2% of `dim_period_anchors.headcount_total`.

## Operational / dev experience — deferred
- [x] **Per-user dev resource naming.** Dev target uses `mode: production` + `${bundle.target}` suffix. Preferred `[dev <user>]` style for ownership visibility + concurrent dev deploys. Deferred because `mode: development` was the source of prior state-drift corruption. Options: literal `${workspace.current_user.short_name}` in `name:` fields; re-enable `mode: development` with strict policy; per-user catalog override. Acceptance: two devs `bundle deploy -t dev` simultaneously without collisions.
- [x] **Split the single `lakehouse` pipeline** into 4–6 per-subject pipelines (low priority). Monolith downsides: one subject's failure breaks the whole run; can't refresh one subject; hard to attribute compute; dev contention. Cross-pipeline deps resolved via Job DAG or expectations.
