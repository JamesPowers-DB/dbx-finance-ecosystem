...
- [x] **UNSPSC taxonomy** — real UNSPSC 25.0 or synthetic 30-category mapping? (Locks how the ML model's predictions encode `unspsc_family_code`.)
- [x] **Lakebase app placement** — standalone project (`projects/spend-analytics`), not embedded. OBO per-request; no shared pool.
- [x] **"Databricks on Databricks" angle** — retain from old script or drop.
- [x] **ML SME** — design doc named TBD. Without one, Phase 2 ML is "best effort" rather than headline.
- [x] **FEIP timing** — Sprint 0 or Sprint 1.
- [x] **Reference-filing privacy** — keep scrubbed or document privately.
- [x] **Schedule for `generate_data`** — one-time or monthly re-randomization.
