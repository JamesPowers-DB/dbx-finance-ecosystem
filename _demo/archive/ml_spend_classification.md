...
- [x] `train_embedding.py` — FMAPI (`databricks-bge-large-en`) embed `line_description` + small head on embedding + tabular. Compare as `@challenger_embedding`. **STILL A STUB.**
...
- [x] UC-registered-model-backed serving endpoint for real-time classification (e.g. inline category suggestions in a supplier-master app).

## Step 7 — Sourcing-strategy outputs
- [x] `gold.vw_sourcing_strategy` — joins `fact_invoices` (classified) with `dim_supplier`, `addressability='Addressable'`: category × segment × supplier-share, top maverick offenders per category, off-contract category spend. One Lakeview tile / Genie space on top.

## Open ML questions
- [x] **UNSPSC mapping** — real UNSPSC codes vs internal 30-category code? Simplest: 1:1 map each of 30 → a chosen UNSPSC family code.
- [x] **Model framework** — LightGBM (interpretable) vs FMAPI-embeddings NN (showcase). Could do both.
- [x] **Confidence threshold** for "managed" classification: probably 0.75+.
